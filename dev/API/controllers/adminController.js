...
// start getUpgradeRequests function
// Fetches all user package upgrade requests.
exports.getUpgradeRequests = (req, res) => {
  db.query('SELECT * FROM upgrade_requests ORDER BY id DESC LIMIT 100', (err, rows) => {
    if (err) return res.status(500).json({ success: false, message: 'DB Error' });
    res.json({ success: true, requests: rows });
  });
};

// start handleUpgradeRequest function
// Admin approves or rejects a user upgrade request.
exports.handleUpgradeRequest = (req, res) => {
  const { id } = req.params;
  ...
  db.query('SELECT * FROM upgrade_requests WHERE id = ?', [id], (err, rows) => {
    ...
    db.query('UPDATE upgrade_requests SET status = ? WHERE id = ?', [newStatus, id], (err) => {
      if (err) return res.status(500).json({ success: false, message: 'Failed to update request status' });

      if (action === 'approve') {
        const pkg = reqItem.requested_package;
        const dailyCredits = pkg === 'enterprise' ? 999999 : 1000;

        db.query('UPDATE users SET package = ?, daily_credits = ? WHERE id = ?', [pkg, dailyCredits, reqItem.user_id], (err) => {
          if (err) console.error('Error updating user package on approval:', err.message);

          logActivity(reqItem.user_id, reqItem.user_email, 'UPGRADE_APPROVED', `Upgrade to "${pkg.toUpperCase()}" approved by Admin (${dailyCredits} credits/day)`);

          res.json({ success: true, message: `Upgrade request approved! User #${reqItem.user_id} is now ${pkg.toUpperCase()} with ${dailyCredits} credits/day.` });
        });
      } else {
        logActivity(reqItem.user_id, reqItem.user_email, 'UPGRADE_REJECTED', `Upgrade to "${reqItem.requested_package.toUpperCase()}" rejected by Admin`);
