...
function extractStreetAddressFromHtml(rawHtml, fullText) {
  ...
  for (const c of containerMatches) {
    ...
    if (/(?:dhaka|chittagong|sylhet|jaipur|mumbai|delhi|bangalore|mirpur|gulshan|banani|shewrapara|uttara|dhanmondi|bashundhara|mohakhali|road|sector|holding|plot|bldg|building|tower|plaza|nagar|marg|bazar)/i.test(text)) {
  ...
  const patterns = [
    /\b((?:\d{1,5}[\/\-A-Za-z0-9]*,?\s+)?(?:Green\s+Delta\s+Aims\s+Tower|GPHOUSE|GP\s*House|SQUARE\s+Centre|House|Plot|Holding|Flat|Level|Floor|Road|Rd|Sector|Block|Avenue|Ave|Lane|Bldg|Building|Tower|Plaza|Complex|Heights|Square|Bhaban|Bazar|Market|[A-Za-z0-9\s.,\(\)#\/-]{3,50})[A-Za-z0-9\s.,\(\)#\/-]{3,80},\s*(?:Mohakhali|Bashundhara|Baridhara|Gulshan|Banani|Dhanmondi|Motijheel|Uttara|Mirpur|Tejgaon|Badda|Shewrapara|Kawran\s*Bazar|Chittagong|Chattogram|Sylhet|Khulna|Rajshahi)[A-Za-z0-9\s.,\/-]{0,50}(?:Dhaka)?(?:[\s,-]*(?:1\d{3}|2\d{3}|3\d{3}|4\d{3}|5\d{3}|6\d{3}|7\d{3}|8\d{3}|9\d{3}))?)\b/i,
...
const SITES = [
  { name: 'Originate Marketing', url: 'https://originatemarketing.com/' },
  { name: 'Harish Finance', url: 'https://harishfinance.in' },
  { name: 'Techtrioz', url: 'https://techtrioz.com' },
  { name: 'Square Pharma', url: 'https://www.squarepharma.com.bd' }
];

async function run() {
  console.log('════════════════════════════════════════════════════════════════════════');
  console.log(' LIVE PHYSICAL ADDRESS VERIFICATION');
  console.log('════════════════════════════════════════════════════════════════════════\n');

  for (const s of SITES) {
    const html = await fetchDirect(s.url, 4000);
